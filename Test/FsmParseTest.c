...
void test_get_transition_list(Fsm_morphological_analyzer_ptr fsm) {
    ...
    test_get_transition_list_single(fsm, 1, "değerlendirmede", "değer+NOUN+A3SG+PNON+NOM^DB+VERB+ACQUIRE^DB+VERB+CAUS+POS^DB+NOUN+INF2+A3SG+PNON+LOC");
...
void test_suffix_list(Fsm_morphological_analyzer_ptr fsm) {
    ...
    test_suffix_list_single(fsm, 1, "değerlendirmede", "NominalRoot(değer)+VerbalRoot(F5PR)(değerlen)+CausativeDHr(değerlendir)+NominalRoot(değerlendirme)+Case1(değerlendirmede)");
